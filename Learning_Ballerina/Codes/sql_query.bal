...
function simpleQuery(mysql:Client mysqlClient) {
    ...
    stream <record {
},     error> resultStream = mysqlClient -> query("Select * from Customers");
...
function countRows(mysql:Client mysqlClient) {
    ...
    stream <record {
},     error> resultStream = mysqlClient -> query("Select count(*) as Total from Customers");
record {|
record {
} value;
...
function typedQuery(mysql:Client mysqlClient) {
    ...
    stream <record {
},     error> resultStream = mysqlClient -> query("Select * from Customers", Customer);
...
function initializeTable() returns sql:Error? {
    ...
    sql:ExecutionResult result = check mysqlClient -> execute("CREATE DATABASE IF NOT EXISTS MYSQL_BBE");
    result = check mysqlClient -> execute("DROP TABLE IF EXISTS " + "MYSQL_BBE.Customers");
    result = check mysqlClient -> execute("CREATE TABLE IF NOT EXISTS " + "MYSQL_BBE.Customers(customerId INTEGER " + "NOT NULL AUTO_INCREMENT, FirstName  VARCHAR(300), LastName  " + "VARCHAR(300), RegistrationID INTEGER," + "CreditLimit DOUBLE, Country  VARCHAR(300), PRIMARY KEY (CustomerId))");
    result = check mysqlClient -> execute("INSERT INTO MYSQL_BBE.Customers " + "(FirstName,LastName,RegistrationID," + "CreditLimit,Country) VALUES ('Peter', 'Stuart', 1, 5000.75, 'USA')");
    result = check mysqlClient -> execute("INSERT INTO MYSQL_BBE.Customers " + "(FirstName,LastName,RegistrationID," + "CreditLimit,Country) VALUES ('Dan', 'Brown', 2, 10000, 'UK')");
